...
{ Characterisation of the custom collation the table index is sorted by.

  The expected values below were lifted byte for byte from the original
  odbec.pas and are kept as literals on purpose: they are the specification,
  not a snapshot of whatever uDxccCollation happens to produce today.
...
const
  { The collation for bytes 0..255, transcribed from odbec.pas chr2bec. }
  ExpectedMapping =
