...
type
  ...
  private
    procedure CompareWholeTable(const Name, TableText: string);
...
procedure TFieldTests.CompareWholeTable(const Name, TableText: string);
var
...
begin
  ...
  Actual   := nil;
  try
    Expected := ReferenceEntries(TableText);
    Actual   := EngineEntries(NewTable(TableText));
    ...
    begin
      ...
      Fail(Format('%s: reference parser and engine disagree ' +
        '(%d vs %d entries)%s',
        [Name, Expected.Count, Actual.Count, Diff]));
    end;
...
begin
  CompareWholeTable('country.tab (plain)', PlainTable);
end;

procedure TFieldTests.ExpandedTableMatchesReferenceParser;
begin
  CompareWholeTable('country.tab (expanded)', ExpandedTable);
end;

procedure TFieldTests.DeletedTableMatchesReferenceParser;
begin
  CompareWholeTable('country_del.tab', DeletedTable);
end;
